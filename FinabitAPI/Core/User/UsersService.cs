...
namespace FinabitAPI.Core.User
{
    public class UsersService
    {
        ...
        public UsersService(
            FinabitAPI.User.UsersRepository repository,
        ...
        public Users GetLoginUser(string userName, string password)
        {
            try
            {
                _logger.LogInformation(
                    "Attempting to get login user for username: {UserName}",
                    userName
                );
                ...
                {
                    _logger.LogInformation(
                        "User login successful for username: {UserName}, UserID: {UserID}",
                        userName,
                        user.ID
                    );
                }
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error occurred while getting login user for username: {UserName}",
                    userName
                );
                throw;
            }
        ...
        public bool ValidateUser(string userName, string password)
        {
            try
            {
                ...
                {
                    _logger.LogInformation(
                        "User validation successful for username: {UserName}",
                        userName
                    );
                }
                else
                {
                    _logger.LogWarning(
                        "User validation failed for username: {UserName}. User ID: {UserID}, HasConnections: {HasConnections}, Status: {Status}",
                        userName,
                        user?.ID,
                        user?.HasConnections,
                        user?.Status
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error occurred while validating user for username: {UserName}",
                    userName
                );
                return false;
            }
        ...
        public Users? GetUserByUsername(string userName)
        {
            try
            {
                ...
                {
                    _logger.LogInformation(
                        "User found for username: {UserName}, UserID: {UserID}",
                        userName,
                        user.ID
                    );
                }
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error occurred while getting user by username: {UserName}",
                    userName
                );
                throw;
            }
        ...
        public Users? GetUserById(int userId)
        {
            try
            {
                ...
                _logger.LogWarning(
                    "GetUserById method not implemented in UsersRepository for UserID: {UserID}",
                    userId
                );
        ...
        public bool CheckUserStatus(string userName)
        {
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "User is not active for username: {UserName}. Status: {Status}, HasConnections: {HasConnections}",
                        userName,
                        user?.Status,
                        user?.HasConnections
                    );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error occurred while checking user status for username: {UserName}",
                    userName
                );
                return false;
            }
