/****** Object:  StoredProcedure [dbo].[usp_CloneTransactionExact_API]    Script Date: 11/13/2025 2:59:37 PM ******/
SET ANSI_NULLS ON
GO
     ...
     BEGIN
	   ...
	   SET @sel  = @sel  + CASE WHEN LEN(@sel)>0 THEN N',' ELSE N'' END +
                CASE 
                     ...
                     WHEN @name = 'Memo'
                          THEN '@Memo'
                     WHEN @name = 'Reference'   -- if your column name is different, change this
                          THEN 'N''Krijuar me ndihmen e AI'''
...
SELECT @OutNewID = MAX(ID) FROM @t;';
        ...
        EXEC sp_executesql
		     @sqlHdr,
			 N'@SourceID INT, @NewDate DATE, @Memo NVARCHAR(500), @OutNewID INT OUTPUT',
			 @SourceID = @SourceID,
			 @NewDate  = @NewDate,
			 @Memo     = @Memo,
...
WHERE p.[TransactionID] = @SourceID;';
        ...
        END
		   ...
		   EXEC [dbo].[spCreateAutomaticJournals]
             @TransactionId = @NewID,
             @UserId        = @UserID; 
