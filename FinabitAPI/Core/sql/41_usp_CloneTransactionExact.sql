...
IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.usp_CloneTransactionExact_API') AND type = 'P')
    EXEC('CREATE PROCEDURE dbo.usp_CloneTransactionExact_API AS BEGIN SET NOCOUNT ON; END');
GO
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SET @sel  = @sel  + CASE WHEN LEN(@sel)>0 THEN N',' ELSE N'' END +
                        CASE WHEN @name IN ('TransactionDate','InvoiceDate','DueDate')
                             THEN '@NewDate'
                             ELSE 'src.' + QUOTENAME(@name) END;
            FETCH NEXT FROM cur INTO @name;
        END
...
SELECT @OutNewID = MAX(ID) FROM @t;';
        ...
        EXEC sp_executesql
             @sqlHdr,
             N'@SourceID INT, @NewDate DATE, @OutNewID INT OUTPUT',
             @SourceID = @SourceID, @NewDate = @NewDate, @OutNewID = @NewID OUTPUT;
