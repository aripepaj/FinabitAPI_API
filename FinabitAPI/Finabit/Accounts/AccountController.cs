...
public class AccountController : ControllerBase
{
    ...
    [HttpGet("getAccounts")]
    [ProducesResponseType(typeof(IReadOnlyList<AccountListItemDto>), StatusCodes.Status200OK)]
    public async Task<ActionResult<IReadOnlyList<AccountListItemDto>>> GetAccountsForPicker(
        [FromQuery] int subGroup,
        CancellationToken ct = default)
    {
        if (subGroup != 6 && subGroup != 7)
            return BadRequest("Invalid subGroup. Use 6 (Arka/Cash) or 7 (Bank).");

        var rows = await _repo.GetAccountsBySubGroupAsync(subGroup, ct);
        return Ok(rows);
    }
